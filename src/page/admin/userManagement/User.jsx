...
const ListUserdata = ListUser.listUser;
...
function User() {
  const [UserListdata, setUserListdata] = useState(SetdataUser())
  //   {
  //     name: 'Nguyễn Văn An ',
  //     datejoin: '2022/7/4',
  //     Userid: '123sscc',
  ...
  function SetdataUser() {
    const newListUser = [];
    ListUserdata.map((value, index) => {
      newListUser.push(
        {
          name: `${value.username}`,
          Userid: `${value._id}`,
          datejoin: `${value.createdAt.slice(0, 10)}`,
          avatar: `${value.avatar}`,
          status: `${value.status}`,
        }
      )
